#!/bin/bash
# CLO tag merge script for FortuneOS
...
BLACKLIST="device/qcom/common vendor/qcom/common \
packages/apps/FortuneCraft packages/resources/devicesettings \
vendor/fortune vendor/fortune/products vendor/gms \
hardware/fortune/compat"
...
for repo in $REPOS; do echo;
    ...
    if ! git fetch -q https://git.codelinaro.org/clo/la/$name $TAG &> /dev/null; then
        ...
        if ! git merge FETCH_HEAD -q --no-edit &> /dev/null; then
...
if $IS_QSSI; then
    ...
    if git -C vendor/fortune commit -aq -m "config: Update CLO revision to $TAG" &> /dev/null; then
